...
from data import (TokenBucketSampler, TokenBucketSamplerAlign, PrefetchLoader, DetectFeatLmdb,
...
def build_aligned_dataloader(dataset_qa, dataset_qar, collate_fn, is_train, opts):
    ...
    if is_train:
        sampler_qa = TokenBucketSamplerAlign([i + j for i, j in zip(dataset_qa.lens, dataset_qar.lens)],
                                        bucket_size=BUCKET_SIZE,
                                        batch_size=batch_size, droplast=is_train, seed=42)
        sampler_qar = TokenBucketSamplerAlign([i + j for i, j in zip(dataset_qa.lens, dataset_qar.lens)],
                                         bucket_size=BUCKET_SIZE,
                                         batch_size=batch_size, droplast=is_train, seed=42)     # 为什么要设置这么一个种子
...
def main(opts):
    ...
    for txt_path, img_path in zip(opts.train_txt_dbs, opts.train_img_dbs):
        ...
        qa_txt_db.id2len = dict((k, qa_txt_db.id2len[k]) for k in qar_txt_db.id2len.keys())  # 保持样本数一致 Ps:这里柯洁写的好像 不对！！！
    ...
    align_fn = get_align_model(
        model_params={'name': 'align4', 'layers': (11, ), 'per_head': False, 'head_average': True},
        sim_fn_params={'name': 'kl'},
        loss_fn_params={'name': 'mrl', 'margin': 0.1}
    )
    ...
    optimizer.zero_grad()
    optimizer.step()
    tot_qa_score, tot_qar_score, tot_score, n_ex = 0, 0, 0, 0
    loss_qa_show, loss_qar_show, loss_align_show = 0, 0, 0
    while True:
        for step, (batch_qa, batch_qar) in enumerate(zip(train_dataloader_qa, train_dataloader_qar)):
            ...
            loss_qa, score_qa, cls_att_dict_qa, box_mask_qa = model(batch_qa, compute_loss=True, output_attention=True, logitorprob=logitorprob)
            loss_qa = loss_qa.mean()
            loss_qar, score_qar, cls_att_dict_qar, box_mask_qar = model(batch_qar, compute_loss=True, output_attention=True, logitorprob=logitorprob)
            loss_qar = loss_qar.mean()

            loss_align, out_align = align_fn(
                cls_att_dict_qa, box_mask_qa, batch_qa['targets'].reshape(-1, 4).argmax(-1),
                cls_att_dict_qar, box_mask_qar, batch_qar['targets'].reshape(-1, 4).argmax(-1),
            )

            loss = loss_qa * 0.5 + loss_qar * 0.5 + loss_align * 0.1

            score_qa = score_qa.view(-1, 4)
            score_qar = score_qar.view(-1, 4)
            curr_qa_score, curr_qar_score, curr_score = compute_accuracies(
                score_qa, batch_qa['targets'].reshape(-1, 4).argmax(-1), score_qar, batch_qar['targets'].reshape(-1, 4).argmax(-1))
            tot_qar_score += curr_qar_score
            tot_qa_score += curr_qa_score
            tot_score += curr_score
            n_ex += batch_qa['input_ids'].size(0) / 4
            ...
            loss_qa_show += copy.deepcopy(loss_qa.detach().item())
            loss_qar_show += copy.deepcopy(loss_qar.detach().item())
            loss_align_show += copy.deepcopy(loss_align.detach().item())
            ...
            if (step + 1) % opts.gradient_accumulation_steps == 0:
                ...
                if global_step % 100 == 0:
                    ...
                    acc_qa = float(tot_qa_score/n_ex)
                    acc_qar = float(tot_qar_score/n_ex)
                    acc_q2ar = float(tot_score/n_ex)
                    tot_qa_score, tot_qar_score, tot_score, n_ex = 0, 0, 0, 0
                    loss_qa_show = loss_qa_show / (opts.gradient_accumulation_steps * 100)
                    loss_qar_show = loss_qar_show / (opts.gradient_accumulation_steps * 100)
                    loss_align_show = loss_align_show / (opts.gradient_accumulation_steps * 100)
                    align_str = ''
                    ...
                    LOGGER.info(f'{tot_ex} examples trained at '
                                ...
                                f' acc_qar-{acc_qar}    '
                                f' acc_q2ar-{acc_q2ar}        '
                                f' loss_qa-{loss_qa_show}    '
                                f' loss_qar-{loss_qar_show}    '
                                f' loss_align-{loss_align_show}        '
                                f'{align_str}')
...
def validate(model, val_loader, align_fn=None, visualize=False):
    ...
    model.eval()
    val_qa_loss, val_qar_loss, val_align_loss = 0, 0, 0
    ...
    for i, batch in enumerate(val_loader):
        ...
        batch_qa, batch_qar = {}, {}
        qids = batch['qids']
        for key, value in batch.items():     # 这个地方需要核验一下
            if len(value) == len(qids) * 8:
                ...
                batch_qa[key] = value
                batch_qar[key] = value
        scores1, cls_att_dict_qa, box_mask_qa = model(batch_qa, compute_loss=False, output_attention=True, logitorprob=logitorprob)
        scores2, cls_att_dict_qar, box_mask_qar = model(batch_qar, compute_loss=False, output_attention=True, logitorprob=logitorprob)
        ...
        val_pbar.update(1)
        loss_align, out_align = align_fn(
            cls_att_dict_qa, box_mask_qa, batch_qa['qa_targets'].squeeze(-1),
            cls_att_dict_qar, box_mask_qar, batch_qar['qar_targets'].squeeze(-1),
        )
        val_align_loss += loss_align
    ...
    LOGGER.info(f"validation finished in {int(tot_time)} seconds, "
                ...
                f"{align_str}"
                f'val_align_loss {val_align_loss:.2f}')
