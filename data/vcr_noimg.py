...
class VcrDataset(VcrDetectFeatTxtTokDataset):
    ...
    def __getitem__(self, i):
        ...
        img_feat = torch.zeros((1, *img_feat.shape[1:]), dtype=img_feat.dtype)
        img_pos_feat = torch.zeros((1, *img_pos_feat.shape[1:]), dtype=img_pos_feat.dtype)
        num_bb = 1
        ...
        for index, input_ids_a in enumerate(input_ids_for_choices):
            ...
            # input_ids = [self.txt_db.cls_] + input_ids_a + [self.txt_db.sep]
            # type_id
            # 0 -- question
            # 1 -- region
            # 2 -- answer
            ...
            txt_type_ids = [0] + type_ids_q + [type_id_for_choice]*(
                len(input_ids_a)+2)
            # type_id_for_choice = 3 if type_ids_q[-1] == 2 else 2
            # txt_type_ids = [0] + [type_id_for_choice]*(
            #     len(input_ids_a)+1)
    ...
    def _get_input_ids(self, txt_dump):
        ...
        for index, input_ids_a in enumerate(input_ids_as):
            ...
            curr_type_ids_qa = [0] + type_ids_q + [2]*(
                len(input_ids_a)+2)
            # curr_input_ids_qa = [self.txt_db.cls_] + input_ids_a + [self.txt_db.sep]
            # curr_type_ids_qa = [0] + [2]*(len(input_ids_a)+1)
            ...
            if (self.split == "val" and index == txt_dump["qa_target"]) or\
                ...
                for input_ids_r in input_ids_rs:
                    ...
                    curr_type_ids_qar = copy.deepcopy(curr_type_ids_qa) +\
                        [3]*(len(input_ids_r)+2)
                    # curr_input_ids_qar = [self.txt_db.cls_] +\
                    #     input_ids_r + [self.txt_db.sep]
                    # curr_type_ids_qar = [0] +\
                    #     [3]*(len(input_ids_r)+1)
